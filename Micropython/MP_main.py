...
pwm_periods = 12                               # PWM Periods per half-wave
f_pwm = int(pwm_periods*f_sine)                 # PWM frequency
scaler = 12                                     # ratio between f_pwm and f_switch
f_switch = scaler * pwm_periods * f_sine *2     # Switching frequency
...
def clock_cb(timer):
    ...
    i = int(cycles / (scaler * 2))      # number of PWM period
    if num == (lut[i]):
        g1.low()
        g2.high()
